...
function autoComplete(e_NIt6) {
	if (e_NIt6) {
		Ajax["call"]('ajax_www_we10_cn.php', 'act=tipword&word=' + e_NIt6, _autoComplete, 'GET', 'JSON');
	}
}
...
$(function(){
	...
	$('.nav02 > div > ul > li > a').on("mouseover",function(){
		$(this).parent().find('.nav02-select').stop().slideToggle();
	});
	$('.nav02 > div > ul > li > a').on("mouseout",function(){
		$(this).parent().find('.nav02-select').stop().slideToggle();
	});
	$('.nav02 > div > ul > li >.nav02-select').on("mouseover",function(){
		$(this).parent().find('.nav02-select').stop().slideToggle();
	});
	$('.nav02 > div > ul > li >.nav02-select').on("mouseout",function(){
		$(this).parent().find('.nav02-select').stop().slideToggle();
	});

	/*$(".nav02 > div > ul > li >.nav02-select").hover(function(){
		$('.nav02 > div > ul > li > a').off("mouseout");
		$(this).slideDown(300);
	},function(){
		$('.nav02 > div > ul > li > a').on("mouseout");
		$(this).parent().find('.nav02-select').slideUp(300);
	});*/

	$('.num .add').click(function(){
		var _val=parseInt($(this).siblings('input').val());
		_val++;
